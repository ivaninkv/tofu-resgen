...
Генератор модулей OpenTofu (`variables.tf`, скелет `main.tf`, пример
`terraform.tfvars.example` и `README.md`) по JSON-схеме провайдера.
...
один заполненный заглушками инстанс переменной. `README.md` — что модуль создаёт,
как его вызвать и что лежит в переменной. Проводка `data`-источников
...
задать, пример значений `terraform.tfvars.example` и `README.md`. С `-all` каждый
ресурс получает свой подкаталог `<каталог>/<ресурс>/`. `-out` и `-module`
взаимоисключающие.
...
Пример копируют в рабочий `terraform.tfvars`. Тот же набор строк обёрнут в блок
`module` в `README.md`, поэтому `-tfvars-optional` действует на оба файла.
...
| `-module` | каталог модуля: `variables.tf`, `main.tf`, `terraform.tfvars.example` и `README.md` (с `-all` — подкаталог на ресурс) |
| `-tfvars-optional` | показать optional-атрибуты закомментированными — в примере tfvars и в примере вызова модуля в `README.md` — со значением, которое они получают по умолчанию |
| `-description` | переопределить описание переменной (оно же открывает `README.md`) |
| `-doc` | выводить описания из схемы: комментариями в `variables.tf` и колонкой в таблицах `README.md` (по умолчанию `true`) |
...
# комплект модуля: variables.tf, main.tf, terraform.tfvars.example и README.md
...
Этот же рендер обёрнут в блок `module` в `README.md`, поэтому `-tfvars-optional`
меняет и пример вызова — подробности в «Правила генерации `README.md`».
...
## Правила генерации `README.md`

Генерируется только `-module`-режимом. Файл отвечает на три вопроса и на этом
заканчивается: что создаёт модуль, как его вызвать, что лежит в переменной.

| Раздел | Из чего |
|---|---|
| `## Описание` | `description` ресурса из схемы (у 241 из 253 ресурсов фикстуры); иначе фраза «Создаёт ресурс `<тип>` провайдера `<адрес>`». Заголовки внутри описания понижаются на два уровня, чтобы outline провайдера не смешивался с разделами файла |
| `## Использование` | блок `module` с примером вызова: `source` — плейсхолдер `git::<repo-url>//<path-to-module>?ref=<ref>` (ни схема, ни каталог модуля не говорят, где он будет опубликован), тело — тот же рендер, что у `terraform.tfvars.example`: только обязательные параметры, а с `-tfvars-optional` ещё и optional-атрибуты закомментированными |
| `## Переменная <имя>` | таблица на каждый уровень вложенности: ресурс с семью уровнями вложенности (в фикстуре — `yandex_connectionmanager_connection`) даёт плоскую таблицу шириной в терминал |

Вводная фраза перед примером следует за флагом: без него «передаются только
обязательные параметры», с ним — что активные строки обязательные, а
закомментированные это optional-атрибуты со значениями, которые модуль применяет
без них. Пример в README и `terraform.tfvars.example` рендерятся одной функцией,
поэтому расходятся они не могут — тест сравнивает их построчно в обоих режимах.

Колонки: `Атрибут | Тип | Обяз. | По умолчанию | Описание`.

- «Тип» — сокращённый: у объекта с полями `object`, `list(object)`, `set(object)`,
  `map(object)`, а сами поля — в отдельной таблице; у объекта без settable-полей
  `object({})`, и таблица не выводится (иначе она была бы из одного заголовка).
- «Обяз.» — обязательность внутри своего объекта; у вложенных таблиц она действует,
  если содержащий объект задан.
- «По умолчанию» — значение из `-defaults` (или `—`), то же, что стоит в
  `optional(T, v)` в `variables.tf`.
- «Описание» — `description` атрибута из схемы; снимается целиком при `-doc=false`.
  Пробелы схлопываются, `|` и `<` экранируются: описания — markdown, часто
  многострочный, и `|` разорвал бы строку таблицы.

Ссылки вида `[dhcp_options](#vpc_subnetdhcp_optionsitem)` ведут в раздел вложенного
объекта; сегменты пути (`item` для `list`/`set`, `key` для `map`) те же, что
в заглушках `terraform.tfvars.example` (`<dhcp_options.item.domain_name>`), поэтому
одно и то же поле находится по одному имени в обоих файлах.

`-check` README не сверяет: это проза, её место — в самой генерации, а расхождение
в ней правится перегенерацией, не отчётом о расхождении со схемой.
...
internal/tfgen/     рендер типов, объект переменной, дефолты, скелет main.tf, пример tfvars, README
...
- **`README.md` не знает, где будет опубликован модуль.** `source` в примере вызова —
  плейсхолдер `git::<repo-url>//<path-to-module>?ref=<ref>`: ни схема, ни каталог
  модуля этого не сообщают, а подставлять чужой адрес репозитория инструмент не
  должен (и в git не заглядывает). Адрес и тег вписываются руками. Описания
  атрибутов переносятся из схемы как есть — если описание несёт собственную ссылку
  вида `[x](#x)`, она остаётся, хотя заголовка `x` в этом файле нет.
