...
| `## Использование` | блок `module` с примером вызова: `source` — плейсхолдер `git::<repo-url>//<path-to-module>?ref=<ref>` (ни схема, ни каталог модуля не говорят, где он будет опубликован), тело — тот же рендер, что у `terraform.tfvars.example`, то есть только обязательные параметры |
